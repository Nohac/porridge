...
- `crates/playground` — toy language built on bowl; serves as the main integration test and usage reference (`cargo test -p playground` runs pipeline consistency tests in `src/tests.rs`). Structured around **language entities** (see `spec/language-entities.md`): `lang/grammar/` is syntax only, `lang/entities/{document,import,definition,namespace}.rs` are vertical slices implementing the stage traits in `lang/entity.rs`, `lang/entities/mod.rs` owns the exhaustive rule→entity lowering dispatch, and `lang/service/` owns request/response facts. Hover uses the **candidate-fact pipeline**, ordered by phases rather than gate markers: service enrichment + per-entity candidate systems (`Phase::Complete`) → priority-picking finalizer (`Phase::Cleanup`). The namespace entity exercises system query joins (`qualify_members`), and enrichment joins requests to files on `FilePath`.
...
- **Joins**: a system query with `Where<Eq<K>>` binds its argument to the unique sibling param reading `&K` and prunes the tuple product to key-equal pairs (fingerprint equality; `K` must be `#[component(hash)]`). Validated at `add_system` (exactly one provider; `View` rejects bound filters). See `spec/joins.md`.
