...
- Ephemeral coordination markers (e.g. the playground's `AstAvailable`) are singleton components emitted from `on_settled` hooks and removed by a `Settle`-phase system (removal-only there; a `Startup`-phase retraction twin covers preemption restarts).
